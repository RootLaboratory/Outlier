...
struct FADSBlurParameters
{
	...
	float FocusDistanceWorld = 0.0f;     // 사이트-마스크 depth-band 판별 기준 거리(cm). 의도적으로 11.0f 고정(ApplyADSBlurRuntimeParameters). DOF 자체의 초점 거리는 별개로 ADSBlurSocketDistance(실측값)를 씀.
	...
	float SightDistanceThreshold = 0.3f; // 위 depth 판별의 허용 오차 폭(cm). 이 값보다 깊이차가 작으면 "조준경 링"으로 인식.

	// Hook B: 화면공간 사이트-튜브 마스크. DoF 사이트-복원 패스를 하드게이트하는 데 써서,
